...
do
    -- create a scope so we don't have to worry about upvalue collisions
        ...
        if group == "party" then
            for i = 0, MAX_PARTY_MEMBERS do
                ...
                if unitName == name then
                    ...
                    unitinfo.name, unitinfo.unitId, unitinfo.Id, unitinfo.lclass, unitinfo.class = unitName, party[i], i, lclass, class
                    return unitinfo
                end
            end
        elseif group == "raid" then
            for i = 1, MAX_RAID_MEMBERS do
                ...
                if unitName == name then
                    ...
                    unitinfo.name, unitinfo.unitId, unitinfo.Id, unitinfo.lclass, unitinfo.class = unitName, raid[i], i, lclass, class
                    return unitinfo
                end
            end
        end
...
function pfUI.api.CreateBackdrop(f, inset, legacy, transp, backdropSetting)
    ...
    if transp and transp < tonumber(ba) then
        ba = transp
    end

    -- use legacy backdrop handling
    if legacy then
        if backdropSetting then
            f:SetBackdrop(backdropSetting)
        end
...
function pfUI.api.BarButtonAnchor(button, basename, buttonindex, barsize, formfactor, iconsize, bordersize, padding,uneven,fillmode)
  ...
  end  local parent = button:GetParent()
  ...
  local y = -bordersize - padding - (row - 1) * step
    ...
    if mode == "cols" then
      local final_col = math.ceil(barsize / rows)
      local count = barsize - (final_col - 1) * rows
      local target_col = (orientation == "LEFT") and 1or cols
      ...
      if count > 0and count < cols and row == target_row and col == 1then
        button._anchor[4] = button._anchor[4] + (cols - count) * step / 2 end
    end
    end
...
function pfUI.api.GetNoNameObject(frame, objtype, layer, arg1, arg2)
    ...
    for _, object in ipairs(objects) do
        local check = true
        if object:GetObjectType() ~= objtype or (layer and object:GetDrawLayer() ~= layer) then
            check = false
        end
        ...
        if check then
            if objtype == "Texture" and object.SetTexture and object:GetTexture() ~= "Interface\\BUTTONS\\WHITE8X8" then
                if arg1 then
                    ...
                    if texture and not string.find(texture, arg1, 1) then
                        check = false
                    end
                end
            ...
            elseif objtype == "FontString" and object.SetText then
                if arg1 then
                    ...
                    if text and not string.find(text, arg1, 1) then
                        check = false
                    end
                end
            ...
            elseif objtype == "Button" and object.GetNormalTexture and object:GetNormalTexture() then
                ...
                if arg2 then
                    local text = object:GetText()
                    if text and not string.find(text, arg2, 1) then
                        check = false
                    end
                end
...
pfUI.api.PLAYER_BUFF_NOT_FOUND = pfUI.expansion == "vanilla" and -1 or 0

-- [ GetPlayerBuffX ] --
-- 
-- 'buffId'      [number]     The id of the buff to retrieve. Starts at 1 and can go up to at least 16. In turtle wow it can go up to 24.
--                            Bare in mind that in Vanilla the GetPlayerBuff() method, that we are wrapping around, had 'buffId' being 0-based
--                            and was changed to be 1-based in TBC. We enforce the TBC behavior here even in Vanilla for the sake of uniformity.
--
-- 'buffFilter'  [string]     The "filter" to use when selecting buffs. Will affect what types of buffs are retrieved. Theoretically, it can
--                            be any combination of HELPFUL, HARMFUL, PASSIVE, CANCELABLE, NOT_CANCELABLE according to comments in the source
--                            code of the original GetPlayerBuff() we are wrapping around.
--
-- return 'buffIndex', 'untilCancelled'
--
--     'buffIndex'          [number]       The index of the buff - if it's not found it will return 'pfUI.api.PLAYER_BUFF_NOT_FOUND' (which is -1 in Vanilla and 0 in TBC onwards)
--     'untilCancelled'     [number]       If 1 this buff will last until it is cancelled (aura, aspect, stealth)
--
-- Example of usage:
--
--     for i=1,32,1 do
...
--     end
--
pfUI.api.GetPlayerBuffX = pfUI.expansion == "vanilla" and (-- vanilla:
        function(buffId, buffFilter)

            local buffIndex, untilCancelled = _G.GetPlayerBuff(buffId - 1, buffFilter)

            -- we intentionally leave 'buffIndex' as is (0-based) for Vanilla   if we translate it into a 1-based index like in TBC we would
            -- break the rest of the Vanilla API surface that expects 'buffIndex' to be 0-based in case of success and -1 in case of failure
            return buffIndex, untilCancelled
        end
) or _G.GetPlayerBuff -- tbc onwards
...
function pfUI.api.TryMemoizedFuncLoadstringForSpellCasts(funcOrStr)
    local msgType = type(funcOrStr)
    if msgType == "function" then --10 order
        return funcOrStr -- return as-is
    end
    ...
    local result = loadstring_cache[funcOrStr]
    if result ~= nil then -- already seen?
        return result
    end

    result = loadstring(funcOrStr)
    ...
    loadstring_cache[funcOrStr] = result --              order
